...
var App = {
    ...
    displayNotification: function (title, msg, stat, sticky){
        UIkit.notification('<span class="uk-text-bold">' + title + '</span><br>' + msg, {status:stat, timeout: sticky === true ? 0 : 6000});
    },
    ...
    displayTokenData : function (tokenData){
        ...
        if (tokenData.access_token){
            ...
            $.ajax ({type:'GET',url:Settings.options.profileurl, headers: {'Authorization': 'Bearer ' + tokenData.access_token}, dataType:'json', contentType:'application/json', success: function (data){
...
var OAuth = {
    ...
    exchangeCode: function (code, isWebMessage, nosecret){
        ...
        var dt = {code: code, grant_type: 'authorization_code', 
                    client_id: settings.oauth_clientid, redirect_uri: App.getCallback()};
        ...
        $.ajax ({type:'POST',url:settings.tokenurl, data:JSON.stringify(dt),   
...
$(function (){
    ...
    if (error.length > 0){
        ...
        if (code.length > 20){
            ...
            var tokenData = {   access_token : App.getParam('access_token'), 
    ...
    window.addEventListener("message", function (event) {
        ...
        if (event.data && event.data.response){
            ...
            if (typeof tokenData.code !== "undefined" && tokenData.code.length > 20) {
                ...
                var tokenData = {   access_token : typeof tokenData.access_token !== "undefined" ? tokenData.access_token : '', 
